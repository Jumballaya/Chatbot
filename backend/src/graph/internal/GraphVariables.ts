...
import type { IOType, IOTypeMap, VariableDef } from "./types";

export class GraphVariables {
  private _definitions: Record<string, VariableDef> = {};
  private _variables: Record<string, IOTypeMap[IOType]> = {};
  public readonly variables: Record<string, IOTypeMap[IOType]>;
  ...
  public addVariable(name: string, type: IOType, initial?: IOTypeMap[IOType]) {
    if (name in this._definitions) {
      throw new Error(`Variable '${name}' already exists`);
    }
    const defaultValue: IOTypeMap[IOType] =
      initial ??
      (type === "number"
        ? 0
        : type === "string"
  ...
  public getVariable<T extends IOTypeMap[IOType]>(name: string): T | undefined {
  ...
  public setVariable(
    name: keyof typeof this._variables,
